...
class k_armed_data():
    ...
    def __init__(self, k):
        self.k = k
        self.reward_means = Gaussian(0, 1, k) # a function()

    def sample_action(self, action_i):
        return Gaussian(self.reward_means[action_i], 1, 1)[0]
...
class q_values():
    ...
    def update_estimates(self, action_selected, r):
        ...
        self.q[action_selected] = self.q[action_selected] + (1 / self.n[action_selected]) * (r - self.q[action_selected])
...
class simple_average_run():
    ...
    def random_action(self):
        return np.random.randint(0, k)  # a random action

    def greedy_action(self, q):
        # Always returns the first max... Has a small bias

        return np.argmax(q)  # the currently optimal action
